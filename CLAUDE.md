...
- **Schema files can drift from code.** Each project's `metadata/*.json` is hand-maintained separately from the JS/GS constants it's supposed to describe (e.g. `apps-script-builder-dashboard/metadata/field-contract.json` vs. `src/Config.gs`'s `FIELD_CONTRACT`). Nothing keeps them in sync. If you change field ownership or add a field, update both, and don't trust the JSON blindly — check the `.gs` source first. Run `npm run check:schema-drift` (root `scripts/check-schema-drift.mjs`) to detect field-set and value drift across all three modern projects; it does not auto-fix anything, only reports. `npm run check:schema-drift:self-test` validates the checker's own comparator logic against fixtures in `scripts/__fixtures__/`.
